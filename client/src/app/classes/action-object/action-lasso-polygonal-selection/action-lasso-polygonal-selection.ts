...
export class ActionLassoSelection extends ActionObject {
    ...
    pathData: Vec2[];
    // tslint:disable-next-line: no-any : reason: cannot know the type of the function
    isPointInPolygon: any;
    lassoPolygonalSelection: LassoPolygonalSelection;
    mouseDownCord: Vec2;

    constructor(pathData: Vec2[], rectangleX: number, rectangleY: number, rectangleWidth: number, rectangleHeight: number) {
        super();
        ...
        this.isPointInPolygon = require('point-in-polygon');
        this.mouseDownCord = { x: 5, y: 9 };
        this.lassoPolygonalSelection = new LassoPolygonalSelection(this.mouseDownCord, {} as ShapeAttribute, 'black', 'black', this.pathData);
    }
        ...
        if (!this.clearImage) {
            ...
            for (let i = startRow; i < limitRow; i++) {
                for (let j = startCol; j < limitCol; j++) {
                    ...
                    // changed this
                    if (this.lassoPolygonalSelection.contains(positionPixel)) {
